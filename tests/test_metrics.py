...
def make_df(n=10, dt=1.0): # sert a creer un DataFrame de test nbr ligne/ intervalle de tmps 
...
def test_vertical_speed_basic(): # test fiabilite code et calculs
    df = make_df()
    vs = compute_vertical_speed(df)
    assert abs(vs.iloc[-1] - 1.0) < 0.2

def test_segment_phases_has_labels(): # valide la segmemtation des phases 
    df = make_df()
...
def test_metrics_do_not_crash(): # verifie que les fonvtions s executent sans erreurs
    df = make_df(120)
    df['vs_mps'] = compute_vertical_speed(df)
